...
def main():
    ...
    # Group findings by KRA
    grouped = defaultdict(list)
    for f in findings:
        grouped[f.kra].append(f)

    # Print in desired order
    order = ["compliance", "security", "reliability", "performance", "cost"]
    for kra in order:
        if kra in grouped and grouped[kra]:
            print(f"\n🔐 {kra.upper()} ISSUES")
            print("-" * 70)
            for f in grouped[kra]:
                print(f"🔴 {f.severity.upper()} | {f.detector_id}")
                print(f"   {f.title}")
    ...
    if not findings:
        print("🎉 Congratulations! No issues found across all KRAs.")
